// Generate a single horizontal composite logo: actual JPEG emblem + "Sefaperp Foods" text + tagline
...
// Wide horizontal logo — emblem on left, "Sefaperp Foods" + tagline on right
function buildHorizontalLogo(height) {
  // Wide aspect ratio: ~5:1 (width:height) to give text plenty of room
  ...
  <circle cx="${emblemX + emblemSize/2}" cy="${emblemY + emblemSize/2}" r="${emblemSize/2 + 1}" fill="none" stroke="#B8943D" stroke-width="1"/>

  <!-- The ACTUAL uploaded logo emblem (circular crop) -->
  ...
  <!-- Company name: "Sefaperp" in burgundy + "Foods" in gold, in a single row -->
  <text x="${textX}" y="${Math.floor(height * 0.48)}" font-family="Playfair Display, Georgia, serif" font-size="${nameFontSize}" font-weight="800" letter-spacing="0">
    <tspan fill="url(#burgundyGrad)">Sefaperp</tspan>
    <tspan fill="url(#goldHorz)" dx="${Math.floor(nameFontSize * 0.20)}">Foods</tspan>
  </text>
  ...
  <!-- Tagline beneath -->
  <text x="${textX}" y="${Math.floor(height * 0.78)}" font-family="Georgia, serif" font-size="${taglineFontSize}" font-style="italic" fill="#6B1C23" letter-spacing="3">Quality · Taste · Trust</text>
</svg>`;
}

(async () => {
  const sizes = [
    { h: 80,  name: 'logo-horizontal-sm.png' },   // small (mobile header)
    { h: 120, name: 'logo-horizontal.png' },       // standard (header)
    { h: 200, name: 'logo-horizontal-lg.png' },    // large (footer / print)
    { h: 50,  name: 'logo-horizontal-xs.png' },    // tiny
  ];

  for (const { h, name } of sizes) {
    const svg = Buffer.from(buildHorizontalLogo(h));
