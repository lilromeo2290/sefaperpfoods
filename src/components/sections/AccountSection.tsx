        ...
        <div className="container mx-auto px-4 md:px-6 max-w-md">
          <Card className="p-8 border-gold/15 shadow-brown">
            <div className="text-center mb-6">
              ...
              <p className="text-brown/60 text-sm mt-1">Sign in or track an order without an account.</p>
            </div>
            <form
            ...
            <p className="text-xs text-brown/50 mt-2">No account needed — just enter your order reference.</p>
          </Card>
        </div>
      </div>
    );
      ...
      <div className="container mx-auto px-4 md:px-6 py-8">
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-8">
          <Card className="p-5 border-gold/15">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-brown/60">Loyalty Tier</p>
          ...
          <Card className="p-5 border-gold/15">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-brown/60">Reward Points</p>
          ...
          <Card className="p-5 border-gold/15">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-brown/60">Total Orders</p>
        ...
        <Tabs value={tab} onValueChange={setTab}>
            ...
            <div className="space-y-3">
              {myOrders.length === 0 ? (
                <Card className="p-8 text-center border-gold/15">
                  <Package className="h-10 w-10 text-brown/30 mx-auto mb-3" />
                  <p className="text-brown/60">No orders yet. Start shopping to see your history here.</p>
                    ...
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      <div>
                        <p className="font-display font-bold text-brown-dark flex items-center gap-2">
                          {o.orderRef}
                          <button onClick={() => { navigator.clipboard?.writeText(o.orderRef); toast.success('Reference copied'); }} className="text-brown/40 hover:text-brown">
              ...
              <Card className="p-5 border-gold/15">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-brown/70 mt-1">12 Mango Street, East Legon, Accra</p>
                    <p className="text-xs text-brown/60 mt-1">GA-123-4567 • +233 247 968 973</p>
                  </div>
          ...
          <TabsContent value="wishlist" className="mt-6">
            <Card className="p-8 text-center border-gold/15">
              <Heart className="h-10 w-10 text-brown/30 mx-auto mb-3" />
              <p className="text-brown/60">Your wishlist is empty.</p>
              <Button className="mt-3 gradient-gold text-brown-dark hover:opacity-90" onClick={() => setView('products')}>
                Discover products
              </Button>
            </Card>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <p className="text-xs text-brown/60 uppercase tracking-wider">Order Reference</p>
          <p className="font-display font-bold text-brown-dark text-xl">{order.orderRef}</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-brown/60 uppercase tracking-wider">Estimated Delivery</p>
            ...
            return (
              <div key={s} className="flex flex-col items-center text-center">
                <div className={cn(
                  ...
                  done ? 'gradient-gold text-brown-dark border-gold' : 'bg-cream text-brown/40 border-gold/30',
                  active && 'ring-4 ring-gold/20 animate-pulse'
                )}>
                  {done ? <CheckCircle2 className="h-4 w-4" /> : i + 1}
                </div>
                <p className={cn('text-[10px] mt-1 font-medium', done ? 'text-brown-dark' : 'text-brown/40')}>
                  {STATUS_LABELS[s]}
                </p>
              </div>
            );
        ...
        <Card className="p-4 border-gold/15 bg-cream/50">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-brown/60">Live tracking enabled • ETA 35 mins</p>
            </div>
      ...
      <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div className="space-y-1">
            {items.map((it: any, i: number) => (
              <div key={i} className="flex justify-between text-xs">
                <span className="text-brown/80">{it.name} ({it.size}) × {it.quantity}</span>
        ...
        <Card className="p-4 border-gold/15">
          ...
          <div className="space-y-1 text-xs">
            <div className="flex justify-between"><span className="text-brown/70">Total</span><span className="font-bold text-brown-dark">GHS {order.total.toFixed(2)}</span></div>
            <div className="flex justify-between"><span className="text-brown/70">Payment</span><span className="text-brown-dark">{order.paymentMethod.toUpperCase()}</span></div>
            <div className="flex justify-between"><span className="text-brown/70">Status</span><span className="text-brand-green font-medium">{order.paymentStatus}</span></div>
            <div className="flex justify-between"><span className="text-brown/70">Delivery to</span><span className="text-brown-dark text-right">{order.city}, {order.region}</span></div>
            {order.lat && order.lng && (
              <div className="flex justify-between"><span className="text-brown/70">GPS</span><span className="text-brown-dark text-right">{order.lat.toFixed(4)}, {order.lng.toFixed(4)}</span></div>
            )}
          </div>
        </Card>
      </div>
